...
export function WebinarsVideos() {
    const [currentSlide, setCurrentSlide] = useState(0);
    const carouselRef = useRef(null);
    const { data: webnairsvideos, isLoading: videosloading, isError: isvideoserror } = useQuery({
    ...
    const totalSlides = Math.ceil((webnairsvideos?.data?.data?.length || 0) / itemsPerPage);
    ...
    const groupedItems = [];
    if (webnairsvideos?.data?.data) {
        for (let i = 0; i < webnairsvideos.data.data.length; i += itemsPerPage) {
            groupedItems.push(webnairsvideos.data.data.slice(i, i + itemsPerPage));
        }
    }
        ...
        <div className="bg-[#f6f6f9]">
            <div className="container flex flex-col gap-5" data-aos="fade-up">
                <div className="flex justify-between">
                    <h2 className='font-bold text-2xl lg:text-5xl'>All On demand-videos</h2>
                </div>
                        ...
                        {groupedItems.map((page, pageIndex) => (
                            <div key={pageIndex} className="w-full flex-shrink-0 px-4">
                                <div className="flex flex-col lg:flex-row gap-5 flex-wrap">
                                    {page.map((i, index) => (
                                        <WebinarsCard key={i.id} webinar={i} category={'videos'} />
                                    ))}
                                </div>
                            </div>
                        ))}
    ...
    return <>
        ...
        <div className="bg-gray-200">
            <div className="container !py-5 flex flex-wrap items-center gap-3">
                <span className='hover:text-hoverText hover:underline cursor-pointer text-grayText text-opacity-80' onMouseDown={(e) => { e.stopPropagation(); navigate(`/webinars`) }}> Webinars</span>
                <FaChevronRight size={10} />
                <span className='cursor-pointer text-grayText text-opacity-100' > On demand-videos</span>
            </div>
        </div>
        <WebinarsVideos />
        <ReadyToTransform />
