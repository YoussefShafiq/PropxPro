...
export function WebinarsEvents() {
    ...
    const { data: webnairsevents, isLoading: eventsloading, isError: iseventserror } = useQuery({
    ...
    const totalSlides = Math.ceil((webnairsevents?.data?.data?.length || 0) / itemsPerPage);
    ...
    const groupedItems = [];
    if (webnairsevents?.data?.data) {
        for (let i = 0; i < webnairsevents.data.data.length; i += itemsPerPage) {
            groupedItems.push(webnairsevents.data.data.slice(i, i + itemsPerPage));
        }
    }
        ...
        <div className="bg-[#f6f6f9]">
            <div className="container flex flex-col gap-5" data-aos="fade-up">
                <div className="flex justify-between">
                    <h2 className='font-bold text-2xl lg:text-5xl'>All Upcoming events</h2>
                </div>
                        ...
                        {groupedItems.map((page, pageIndex) => (
                            <div key={pageIndex} className="w-full flex-shrink-0 px-4">
                                <div className="flex flex-col lg:flex-row gap-5 flex-wrap">
                                    {page.map((i, index) => (
                                        <WebinarsCard key={i.id} webinar={i} category={'events'} />
                                    ))}
                                </div>
                            </div>
                        ))}
    ...
    return <>
        ...
        <div className="bg-gray-200">
            <div className="container !py-5 flex flex-wrap items-center gap-3">
                ...
                <span className='cursor-pointer text-grayText text-opacity-100' >Upcoming events</span>
            </div>
        </div>
        <WebinarsEvents />
        <ReadyToTransform />
